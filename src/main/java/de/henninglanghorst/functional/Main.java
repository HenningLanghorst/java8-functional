...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        doInDatabase(
                cp::getConnection,
                databaseUpdate(
                        statement("insert into Person values (?, ?, ?, ?);", 1, "Carl", "Carlsson", Date.valueOf("1972-04-02"))))
                .handleResult(objects -> System.out.println("Success " + objects), Throwable::printStackTrace);

        doInDatabase(
                cp::getConnection,
                databaseUpdate(
                        statement("insert into Person values (?, ?, ?, ?);", 2, "Lenny", "Leonard", Date.valueOf("1981-04-02"))))
                .handleResult(objects -> System.out.println("Success " + objects), Throwable::printStackTrace);
